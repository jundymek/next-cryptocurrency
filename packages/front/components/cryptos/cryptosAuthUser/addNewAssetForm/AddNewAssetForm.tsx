...
interface AddNewAssetFormProps {
  cryptos: CryptoData[];
  setVisibleAssets: React.Dispatch<React.SetStateAction<Asset[]>>;
  setNotInAssets: React.Dispatch<React.SetStateAction<CryptoData[]>>;
}

const AddNewAssetForm = React.memo<AddNewAssetFormProps>(
  ({ cryptos, setVisibleAssets, setNotInAssets }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      if (token) {
        ...
        setVisibleAssets((prevState) => [...prevState, payload]);
        setNotInAssets((prevState) =>
          prevState.filter((item) => item.firstCurrency != payload.currencyName),
        );
      }
    };
